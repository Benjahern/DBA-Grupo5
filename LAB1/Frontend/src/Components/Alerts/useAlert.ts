/**
 * @fileoverview Sistema de estado global para notificaciones (Alertas/Toasts).
 * * Utilidad: Este archivo provee un "composable" (useAlert) que permite a cualquier
 * componente de la aplicación mostrar, ocultar y reaccionar al estado de una
 * alerta flotante. Al mantener el estado reactivo fuera de la función exportada,
 * actúa como un "Singleton", asegurando que toda la app comparta la misma instancia de la alerta.
 */

import { reactive } from 'vue';

// Define los tipos visuales permitidos para las alertas
export type AlertSeverity = 'success' | 'error' | 'info' | 'warning';

// Define la estructura exacta del estado de la alerta
 ...
 */
const alerts = reactive<AlertState>({
    isVisible: false,
    message: '',
    severity: 'info',
    autoHideMs: 4000,

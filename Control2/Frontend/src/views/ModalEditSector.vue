...
const handleSubmit = async () => {
  ...
  loading.value = true;
  try {
    await api.put(`/api/sectors/${props.sector.id}`, {
      name: form.value.name
    });
    show({ message: 'Sector actualizado correctamente.', severity: 'success', autoHideMs: 3000 });
    emit('updated');
    emitClose();
  } catch (err) {
    console.error('Error updating sector:', err);
    error.value = 'No se pudo actualizar el sector.';
    show({ message: 'Error al actualizar el sector.', severity: 'error', autoHideMs: 4000 });
  } finally {
    loading.value = false;
  }
};
